...
Fits payoff weights w1, w2, w3, w4, w5 and temperature using maximum likelihood
...
def calibrate_weights(
    ...
    """
    Calibrate w1, w2, w3, w4, w5, temperature by minimizing BCE on train.
    Uses grid search over weight combinations.
    quick=True: coarser grid for faster runs.
    """
    train_vt = train[train[vote_col].isin(["Voor", "Tegen"])].copy()
    if train_vt.empty:
        return {"w1": 0.45, "w2": 0.25, "w3": 0.1, "w4": 0.1, "w5": 0.1, "temperature": 1.0}
    ...
    best_loss = float("inf")
    best_weights = {"w1": 0.45, "w2": 0.25, "w3": 0.1, "w4": 0.1, "w5": 0.1, "temperature": 1.0}

    # Grid search: policy, coalition, electoral, reciprocity, discipline
    if quick:
        w1_candidates = [0.4, 0.5]
        w2_candidates = [0.2, 0.3]
        w3_candidates = [0.1]
        w4_candidates = [0.1]
        w5_candidates = [0.0, 0.1]
        temp_candidates = [1.0, 1.5]
    else:
        w1_candidates = [0.35, 0.4, 0.45, 0.5, 0.55]
        w2_candidates = [0.2, 0.25, 0.3, 0.35]
        w3_candidates = [0.05, 0.1, 0.15]
        w4_candidates = [0.05, 0.1, 0.15]
        w5_candidates = [0.0, 0.05, 0.1, 0.15, 0.2]  # discipline (DPBD)
                ...
                for w4 in w4_candidates:
                    for w5 in w5_candidates:
                        ...
                        payoff_voor, payoff_tegen = compute_payoffs_batch(
                            ...
                            w1=w1n, w2=w2n, w3=w3n, w4=w4n, w5=w5n,
